...
contract("UserDescriptors test", async accounts => {
  ...
  it("should return the correct pair of values per unit", async () => {
    ...
    await contractInstance.insertValue(expectedVal1.unit, expectedVal1.val, {
      from: accounts[0]
    });
    await contractInstance.insertValue(expectedVal2.unit, expectedVal2.val, {
      from: accounts[0]
    });
    await contractInstance.insertValue(expectedVal3.unit, expectedVal3.val, {
      from: accounts[0]
    });
  ...
  it("should return a list full of values if values were inserted", async () => {
    ...
    for (let i = 0; i < expectedList.length; ++i) {
      await contractInstance.insertValue(key, expectedList[i], {
        from: accounts[0]
      });
    }
